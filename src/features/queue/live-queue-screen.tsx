...
import {
  type ClinqueNotification,
  useNotifications,
} from "@/features/notifications/notification-context";
...
export function LiveQueueScreen() {
  ...
  const [visibleAlert, setVisibleAlert] = useState<ClinqueNotification | null>(
    null,
  );
      ...
      <View style={styles.screen}>
        <SafeAreaView style={styles.loadingSafeArea}>
          ...
          <Text style={styles.loadingCaption}>
            Restoring your latest clinic status…
          </Text>
        </SafeAreaView>
      </View>
    );
        ...
        <SafeAreaView style={styles.emptySafeArea}>
          <View style={styles.emptyCard}>
            <View style={styles.emptyIcon}>
              <Icon
                name={{
                  ios: "person.line.dotted.person.fill",
                  android: "groups",
                  web: "groups",
                }}
                size={29}
            ...
            <Text style={styles.emptyCaption}>
              Check in from Journey when you arrive at the clinic.
            </Text>
            <Pressable
  ...
  const progress =
    called || consulting || completed
      ? 1
      : Math.min((5 - queue.position) / 5, 0.9);
  ...
  const heroValue = completed
    ...
    : consulting
      ...
      : called
        ? "Now"
        : `#${queue.position}`;
  const heroCaption = completed
    ? "Your visit summary and care instructions are ready."
    : consulting
      ? `${appointment.doctorName} is attending to you in Consultation Room 3.`
      : called
        ? "Please proceed to Consultation Room 3."
        : `${queue.position} patient${queue.position === 1 ? "" : "s"} ahead of you`;
        ...
        >
          {visibleAlert && (
            <View
              style={[
                styles.alertBanner,
                visibleAlert.type === "doctor-ready" && styles.alertBannerReady,
              ]}
            >
              <View style={styles.alertBannerIcon}>
                <Icon
                  name={
                    visibleAlert.type === "doctor-ready"
                      ? {
          ...
          <View
            style={[
              styles.queueHero,
              (called || consulting || completed) && styles.queueHeroCalled,
            ]}
          >
            ...
            <Text
              style={[
                ...
                consulting && styles.positionValueCompact,
              ]}
            >
              {heroValue}
            </Text>
              ...
              <View>
                ...
                <Text style={styles.waitValue}>
                  {completed
                    ...
                    : consulting
                      ? `Started ${formatTime(queue.consultationStartedAt ?? queue.lastUpdatedAt)}`
                      : called
          ...
          <View style={styles.locationCard}>
            ...
            <View style={styles.locationContent}>
              <Text style={styles.locationClinic}>
                {appointment.clinicName}
              </Text>
              <Text style={styles.locationCaption}>
                {called || consulting
                  ? "Consultation Room 3 · Level 2"
                  : completed
                    ? "Visit completed"
                    : "Waiting area · Level 2"}
              </Text>
            </View>
            <View style={styles.arrivedPill}>
              ...
              <Text style={styles.arrivedText}>
                {completed ? "COMPLETE" : consulting ? "IN CARE" : "ARRIVED"}
              </Text>
            </View>
          </View>
          ...
          <View style={styles.progressCard}>
            ...
            <QueueStep
              ...
              complete
              label="Checked in"
              time={formatTime(queue.checkedInAt)}
            />
            <QueueStep
              active={!called && !consulting && !completed}
              complete={called || consulting || completed}
              label="Waiting for doctor"
              time={
                called || consulting || completed ? "Completed" : "In progress"
              }
            />
            <QueueStep
              active={called || consulting}
              complete={completed}
              label="Consultation"
              time={
                completed
          ...
          <View style={styles.guidanceCard}>
            ...
            <View style={styles.guidanceContent}>
              <Text style={styles.guidanceTitle}>
                {completed
                  ? "Continue to your visit summary"
                  : consulting
                    ? "Your consultation is underway"
                    : called
                      ? "Please enter Consultation Room 3"
                      : "You can step away briefly"}
              </Text>
              <Text style={styles.guidanceCaption}>
                {completed
                  ? "Review your diagnosis, medication, follow-up notes, and recovery checklist in Journey."
                  : consulting
                    ? "After the clinician completes your visit, Clinque will make your summary and care instructions available."
                    : called
                      ? "The doctor is ready for you. Bring your identification and any relevant medication."
                      : "Clinque will alert you when two patients remain. Stay near the clinic."}
              </Text>
            </View>
          </View>
            ...
            <View style={styles.demoHeader}>
              <View>
                <Text style={styles.demoLabel}>
                  {isDemo ? "PORTFOLIO DEMO" : "SECURE REALTIME"}
                </Text>
...
const styles = StyleSheet.create({
  ...
  positionValueCompact: { fontSize: 38, letterSpacing: -1 },
  heroCaption: {
    marginTop: 3,
    color: "#D7EFEA",
    fontSize: 10,
